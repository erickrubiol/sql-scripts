...
(
 SELECT * FROM (
 SELECT row_number() over (order by COUNT(id_art) desc) as rank_ventas
 ,id_art
 FROM compras
 GROUP BY id_art
 ) top7
...
(
 SELECT * FROM (
 SELECT id_clasif, compras.id_art, STR(id_clasif) + '_' + TRIM(STR( row_number() over (PARTITION BY id_clasif order by COUNT(compras.id_art)
desc))) AS id_categoria,
...
promos_cliente --numero de articulos en promoción que compro el cliente
AS
(
 SELECT id_cte,
 COUNT(*) AS max_prom
...
top_categoria_cliente --categoria que más compró el cliente de artículos en promoción
AS
(
 SELECT id_cte, id_clasif FROM (SELECT id_cte, id_clasif
 ...
 GROUP BY id_cte, id_clasif) test where rank_promo = 1
),
top_por_cliente -- top de artículos que compró el cliente de artículos en promoción
AS
(
 SELECT id_cte, id_art, row_number() over (PARTITION BY id_cte order by COUNT(id_art) desc) as rank_por_cliente
 , id_cte + '_' + TRIM(STR( id_art )) id_top
 FROM compras LEFT JOIN Promos ON compras.id_art = Promos.ID
 WHERE Promos.ID IS NOT NULL
...
AS
(
 SELECT top_por_cliente.id_cte + '_' + TRIM(STR(id_art)) AS exist,
...
TRIM(STR(top7_categoria.id_art))
, excluir.id_excluir
),
final
AS
